# CD Nodejs in VPS server with github action
...
#### Create CD pipeline
- In the back-end project create file `.github/workflows/staging.yml` and paste the below deployment code : 
```
...
jobs:
  ...
  deploy:
    name: 'Deploy to backend BACKEND'
    ...
    steps:
      ...
      - name: Deploying into server
        run: |
          ssh server 'cd /home/ubuntu/mice.ditinex.com/MICE-backend && git reset --hard && git pull origin staging && npm install && pm2 restart mice.ditinex.com-BE --time && pm2 flush mice.ditinex.com-BE'
